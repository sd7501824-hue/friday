...
class _UnixDefaults(PlatformDirsABC):  # noqa: PLR0904
    ...
    def _site_applications_dirs(self) -> list[str]:
        return [
            os.path.join(p, "applications") for p in ["/usr/local/share", "/usr/share"]
        ]  # noqa: PTH118
...
class Unix(XDGMixin, _UnixDefaults):
    ...
    def user_applications_dir(self) -> str:
        ...
        return (
            self.site_applications_dir
            if self._use_site
            else super().user_applications_dir
        )
...
def _get_user_dirs_folder(key: str) -> str | None:
    ...
    """
    config_home = os.environ.get("XDG_CONFIG_HOME", "").strip() or os.path.expanduser(
        "~/.config"
    )  # noqa: PTH111
